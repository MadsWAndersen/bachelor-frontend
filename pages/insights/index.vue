<template>
	<div v-if="cmsContent">
		<HeroHeader :headline="cmsContent.insightsHeadline" :heroText="cmsContent.insightsDescription" />
		...
		<div v-if="cmsContent.childrenData._embedded.content">
			<div v-for="(content, index) in cmsContent.childrenData._embedded
				...
				<div v-for="(childContent, index) in content.childrenData
					._embedded.content" :key="index" class="mb-4">
					<div v-if="childContent.highligted === true">
						<HighlightCard :title="childContent.headline" :bodyText="childContent.bodyText"
		...
		<div class="container-row lg:mb-10 mb-6 lg:mt-10">
			<div
				...
				<input v-model="searchTerm" type="text" placeholder="Search here.."
					class="border-0 pl-5 outline-none w-full" />
			</div>
		</div>

		<div class="container-row">
			<sectionCard v-for="(card, index) in filteredCards" :key="index" :url="card._url"
				:headline="card.insightHeadline" :description="card.teaserText" :image="card.image[0]._url"
				:icon="card.icon?.src" :style="'dark'">
			</sectionCard>
		</div>
	</div>
</template>
