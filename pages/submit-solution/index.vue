<template>
	<div class="container-row flex lg:justify-center">
		<div class="col-span-12 lg:w-1/2 md:w-2/3 w-full">
			<HeroHeader headline="Submit solution" heroText="" heroBreadCrumbs="Submit-solution" />
			<div class="container-row lg:flex justify-center">
				<div class="col-span-12 w-full">
					...
					<p class="text-m text-um-black pb-6">
						Here you will be able to submit a new solution for at
						workflow or a know issue with the accounting
					</p>

					<div class="md:flex lg:row w-full mb-5">
						<div class="md:pr-6 w-full">
							<p class="w-full text-m font-bold text-um-blue pt-6 mr-5">
								Select the category
							</p>
							<select id="parentId"
								...
								<option v-for="(
										content, index
									) in localStorageContent" :key="index"
						...
						<div class="w-full">
							...
							<div class="w-full flex">
								...
								<button id="versionBtn"
									...
									@click="addVersion()">
									<span class="text-um-blue">Add</span>
								</button>
							</div>
							<div class="w-full flex flex-wrap max-w-sm">
								<span v-for="(version, index) in versions" :key="index"
									class="flex row w-[100px] relative justify-center h-[30px] mr-4 mb-1 items-center bg-um-blue rounded-xs text-xs text-um-palepink font-bold">
									{{ version }}<span
										class="bg-um-white text-center cursor-pointer select-none absolute w-[20px] h-[20px] bg-red-400 rounded-full border-[1px] border-um-palepink top-[-5px] right-[-5px]"
										@click="removeVersion(index)">x</span>
								</span>
							</div>
						</div>
					</div>
...
const addVersion = () => {
	...
	if (version.value) {
		versions.value.push(version.value)
		document.querySelector('#versions').value = ''
	} else {
		version.placeholder = "Please enter a version";
	}
}
