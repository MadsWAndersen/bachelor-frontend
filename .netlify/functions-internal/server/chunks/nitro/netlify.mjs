...
function hasProtocol(inputString, opts = {}) {
	...
	return (
		...
		(opts.acceptRelative
			? PROTOCOL_RELATIVE_REGEX.test(inputString)
			: false)
	)
}
...
function splitCookiesString(cookiesString) {
	...
	while (pos < cookiesString.length) {
		...
		if (!cookiesSeparatorFound || pos >= cookiesString.length) {
			cookiesStrings.push(
				cookiesString.slice(start, cookiesString.length),
			)
		}
	}
...
class H3Event {
	...
	get handled() {
		return (
			this._handled ||
			this.node.res.writableEnded ||
			this.node.res.headersSent
		)
	}
...
const inlineAppConfig = {
	nuxt: {
		buildId: '9b9a48b8-4e2f-4f37-9c7c-99fae2ce6c2a',
	},
}
...
function _getEnv(key) {
	...
	return destr(
		process.env[ENV_PREFIX + envKey] ??
			process.env[ENV_PREFIX_ALT + envKey],
	)
}
			...
			switch (responseType) {
				...
				default: {
					context.response._data =
						await context.response[responseType]()
				}
			}
		}
...
function isNativeFunction(f) {
	...
	return (
		Function.prototype.toString.call(f).slice(-nativeFuncLength) ===
		nativeFunc
	)
}
	...
	stringify(wordArray) {
		...
		for (let i = 0; i < wordArray.sigBytes; i += 3) {
			const byte1 =
				(wordArray.words[i >>> 2] >>> (24 - (i % 4) * 8)) & 255
			const byte2 =
				(wordArray.words[(i + 1) >>> 2] >>> (24 - ((i + 1) % 4) * 8)) &
				255
			const byte3 =
				(wordArray.words[(i + 2) >>> 2] >>> (24 - ((i + 2) % 4) * 8)) &
				255
			const triplet = (byte1 << 16) | (byte2 << 8) | byte3
			for (
				let j = 0;
				j < 4 && i * 8 + j * 6 < wordArray.sigBytes * 8;
				j++
			) {
				base64Chars.push(
					keyStr.charAt((triplet >>> (6 * (3 - j))) & 63),
				)
			}
		}
...
function createStorage(options = {}) {
	...
	const runBatch = (items, commonOptions, cb) => {
		...
		return Promise.all(
			[...batches.values()].map((batch) => cb(batch)),
		).then((r) => r.flat())
	}
	const storage = {
		// Item
		...
		async getKeys(base, opts = {}) {
			...
			for (const mount of mounts) {
				...
				const keys = rawKeys
					...
					.filter(
						(key) => !maskedMounts.some((p) => key.startsWith(p)),
					)
				allKeys.push(...keys)
				maskedMounts = [
					mount.mountpoint,
					...maskedMounts.filter(
						(p) => !p.startsWith(mount.mountpoint),
					),
				]
			}
			return base
		...
		async unmount(base, _dispose = true) {
			...
			context.mountpoints = context.mountpoints.filter(
				(key) => key !== base,
			)
...
async function rmRecursive(dir) {
	...
	await Promise.all(
		entries.map((entry) => {
			const entryPath = resolve(dir, entry.name)
			if (entry.isDirectory()) {
				return rmRecursive(entryPath).then(() =>
					promises.rmdir(entryPath),
				)
			} else {
		...
		if (PATH_TRAVERSE_RE.test(key)) {
			throw createError(
				...
				`Invalid key: ${JSON.stringify(
					key,
				)}. It should not contain .. segments`,
			)
		}
...
storage.mount(
	...
	unstorage_47drivers_47fs_45lite({
		driver: 'fsLite',
		base: 'C:\\Users\\madsw\\dev\\umbraco\\.data\\kv',
	}),
)
...
function hasReqHeader(event, name, includes) {
	...
	return (
		value &&
		typeof value === 'string' &&
		value.toLowerCase().includes(includes)
	)
}
...
const errorHandler = async function errorhandler(error, event) {
	...
	if (error.unhandled || error.fatal) {
		const tags = [
			...
			error.fatal && '[fatal]',
			Number(errorObject.statusCode) !== 200 &&
...
export {
	...
	withQuery as w,
	parseQuery as x,
	withTrailingSlash as y,
	withoutTrailingSlash as z,
}
